...
dataF = 2*abs(fft(mean(data, 3), [], 2) / EEG.pnts);

% vector of frequencies
hz = linspace(0, EEG.srate/2, floor(EEG.pnts/2) +1);

% frequency cutoffs in Hz and indices
f_start = dsearchn(hz', sinefreq1);
f_end = dsearchn(hz', sinefreq2);

% power in first spectral window
dataf_mean = mean(dataF(:,f_start), 2);
dataf2_mean = mean(dataF(:,f_end), 2);


% topographical plots of dipole projections and band-specific power.
figure(5), clf
subplot(121)
topoplotIndie(dataf_mean, EEG.chanlocs,'numcontour',0,'electrodes','off','shading','interp');
%set(gca,'clim',[-1 1]*40)
title('Signal dipole projection')
subplot(122)
topoplotIndie(dataf2_mean, EEG.chanlocs,'numcontour',0,'electrodes','off','shading','interp');
%set(gca,'clim',[-1 1]*40)
title('Signal dipole projection')
